...
// #region Gene conditions
    ...
    bool CondObstacle1(in uint2 cellPos) { // has obstacle in forward direction
        ...
        uint2 target = ShiftCoord(cellPos, cellDir);
        uint tidx = target.y * _Width + target.x;
    ...
    bool CondObstacle2(in uint2 cellPos) { // has obstacle in left direction
        ...
        uint2 target = ShiftCoord(cellPos, left);
        uint tidx = target.y * _Width + target.x;
        return _Cells[tidx].cellType != 0;
    }
...
bool CommandMove(in uint2 cellPos/* , uint moveDir */) {
    ...
    if (_Cells[cellIdx].cellType < 5 || _Cells[targetIdx].cellType > 0) return false;
...
bool CommandGrow(in uint2 cellPos) {
    ...
    directionTypes[0] = UnpackInt1(growDirections) % 32;
    directionTypes[1] = UnpackInt2(growDirections) % 32;
    directionTypes[2] = UnpackInt3(growDirections) % 32;
    directionTypes[3] = UnpackInt4(growDirections) % 32;
...
bool ExecuteCommand(in uint2 cellPos, in uint commandId) {
    uint cellIdx = cellPos.y * _Width + cellPos.x;
    // Cell cell = _Cells[cellIdx];

    [branch] // Принуждает GPU использовать реальное ветвление
    switch(commandId) {
        case 0: return CommandMove(cellPos);
        case 1: return CommandRotate(cellPos, 1);
        case 2: return CommandGrow(cellPos);
    }
...
void ExecuteGene(in uint2 cellPos, in Gene gene)
{
    ...
    // read conditions
    bool check1 = CheckGeneCondition(cellPos, UnpackInt1(gene.conditions), gene.condParam1);
